...
library(robustbase)
data(alcohol)
plot(logSolubility ~ SAG, data = alcohol, xlab = 'Surface Area', ylab = 'Measured Solubility' )
...
```{r, echo=FALSE}
model2 <- lm(logSolubility ~ ., data = alcohol)
pred <- predict.lm(model2, alcohol)
plot(pred, alcohol$logSolubility, xlab = 'Predicted Solubility', ylab = 'Measured Solubility')
```
